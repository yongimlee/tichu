...
**Bot difficulty (`Player.difficulty: BotDifficulty`).** The host picks a tier when adding a bot (`room:addBot` payload). `'normal'` = the heuristic `botMove` (synchronous). `'expert'` = determinized Monte-Carlo search (`server/src/pimc.ts`, `pimcChooseMove`): it deals the unseen cards to opponents by known counts (never peeking), rolls each candidate move out to the hand's end with `botMove` for everyone, and picks the best average team-point margin under a wall-clock budget. Measured ~70% / +48 pts vs the heuristic. Because the search is CPU-bound (~0.5s), it runs in a **worker thread pool** (`pimcPool.ts` + `pimcWorker.ts`) so the event loop stays responsive; `BotDriver.step` is async for the expert path and re-validates the chosen move against the live state after the await (a human can bomb out of turn while it thinks), falling back to a fresh `botMove` then a pass. Tunable via env: `PIMC_WORKERS`, `PIMC_BUDGET_MS`. Workers load as `.ts` because the server runs under `tsx` (which propagates to worker threads); an offline human's stand-in always uses `'normal'`. Reserved: easier tiers below `'normal'` for a future bot-weakening pass.
